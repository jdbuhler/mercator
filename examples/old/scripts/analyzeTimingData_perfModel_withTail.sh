...
# input fields for kernel-level timing: timerType ($2): 0, mainLoopTime ($4),
#  schedulerTime ($5), firingQueue ($9), tailTime ($12)
...
#  Fields 11 & 14
          ...
          $2 == 0 \
          { totalKernelTime += $4; \
            totalSchedulingTime += $6; \
            totalFiringQueueTime += $9; \
            totalTailTime += $12; \
            if ($4 > 0) { \
              blockCount += 1;  \
            } \
          ...
          END { totalFiringRatio = totalFiringQueueTime/totalKernelTime; \
                ...
                avgTotalSchedulingTime = totalSchedulingTime/blockCount; \
                avgTotalFiringQueueTime = totalFiringQueueTime/blockCount; \
                avgTotalTailTime = totalTailTime/blockCount; \
                printf "Num nonempty blocks, avg total mainKernel time, avg total scheduling time, avg total firing time, avg total tail time\n";  \
                printf "%d, %.2f, %.2f, %.2f, %.2f\n", \
                  blockCount, avgTotalKernelTime, avgTotalSchedulingTime, \
                  avgTotalFiringQueueTime, avgTotalTailTime;  \
