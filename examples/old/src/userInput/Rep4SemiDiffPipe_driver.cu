#ifndef __REP4SEMIDIFFPIPE_DRIVER_CU
#define __REP4SEMIDIFFPIPE_DRIVER_CU
...
#if MAPPING_1TO1
  #include "../codegenInput/Rep4SemiDiffPipe_1to1map.cuh"
  #define APP_TYPE Rep4SemiDiffPipe_1to1map
//#elif MAPPING_1TO2
...
#define PRINT_INPUT_BUFFER_REP4SEMIDIFFPIPE 0
#define PRINT_OUTPUT_BUFFERS_REP4SEMIDIFFPIPE 0

void run_rep4SemiDiffPipe()
{
...
//  constexpr int GAMMA = 2;
...
#if PRINT_INPUT_BUFFER_REP4SEMIDIFFPIPE 
  // print input buffer contents
  printf("Rep4SemiDiffPipe, InputBuffer (%p):\n", inBuffer);
  for(int i=0; i < IN_BUFFER_CAPACITY; ++i)
  {
  ...
  // create app object
  APP_TYPE* rep4SemiDiffPipe = new APP_TYPE();
...
//    printf("Setting upper bound of node %d to %d\n", i, upperBd);
  ...
  rep4SemiDiffPipe->A->set_nodeUserData(APP_TYPE::A::Node::A1node, myNodeData[0]);
  rep4SemiDiffPipe->B->set_nodeUserData(APP_TYPE::B::Node::B1node, myNodeData[1]);
  rep4SemiDiffPipe->C->set_nodeUserData(APP_TYPE::C::Node::C1node, myNodeData[2]);
  rep4SemiDiffPipe->D->set_nodeUserData(APP_TYPE::D::Node::D1node, myNodeData[3]);
  rep4SemiDiffPipe->E->set_nodeUserData(APP_TYPE::E::Node::E1node, myNodeData[4]);
  rep4SemiDiffPipe->A->set_nodeUserData(APP_TYPE::A::Node::A2node, myNodeData[0]);
  rep4SemiDiffPipe->B->set_nodeUserData(APP_TYPE::B::Node::B2node, myNodeData[1]);
  rep4SemiDiffPipe->C->set_nodeUserData(APP_TYPE::C::Node::C2node, myNodeData[2]);
  rep4SemiDiffPipe->D->set_nodeUserData(APP_TYPE::D::Node::D2node, myNodeData[3]);
  rep4SemiDiffPipe->E->set_nodeUserData(APP_TYPE::E::Node::E2node, myNodeData[4]);
  rep4SemiDiffPipe->A->set_nodeUserData(APP_TYPE::A::Node::A3node, myNodeData[0]);
  rep4SemiDiffPipe->B->set_nodeUserData(APP_TYPE::B::Node::B3node, myNodeData[1]);
  rep4SemiDiffPipe->C->set_nodeUserData(APP_TYPE::C::Node::C3node, myNodeData[2]);
  rep4SemiDiffPipe->D->set_nodeUserData(APP_TYPE::D::Node::D3node, myNodeData[3]);
  rep4SemiDiffPipe->E->set_nodeUserData(APP_TYPE::E::Node::E3node, myNodeData[4]);
  rep4SemiDiffPipe->A->set_nodeUserData(APP_TYPE::A::Node::A4node, myNodeData[0]);
  rep4SemiDiffPipe->B->set_nodeUserData(APP_TYPE::B::Node::B4node, myNodeData[1]);
  rep4SemiDiffPipe->C->set_nodeUserData(APP_TYPE::C::Node::C4node, myNodeData[2]);
  rep4SemiDiffPipe->D->set_nodeUserData(APP_TYPE::D::Node::D4node, myNodeData[3]);
  rep4SemiDiffPipe->E->set_nodeUserData(APP_TYPE::E::Node::E4node, myNodeData[4]);

  // associate buffers with nodes
  rep4SemiDiffPipe->sourceNode->set_inBuffer(inBuffer);
  rep4SemiDiffPipe->sinkNodeAccept1->set_outBuffer(outBufferAccept);
  rep4SemiDiffPipe->sinkNodeAccept2->set_outBuffer(outBufferAccept);
  rep4SemiDiffPipe->sinkNodeAccept3->set_outBuffer(outBufferAccept);
  rep4SemiDiffPipe->sinkNodeAccept4->set_outBuffer(outBufferAccept);

  // run main function
  rep4SemiDiffPipe->run();

  std::cout << "REP-4-SEMIDIFF-PIPE APP FINISHED.\n" ;
...
#if PRINT_OUTPUT_BUFFERS_REP4SEMIDIFFPIPE
  std::cout << " Output buffer: \n" ;

  printf("Rep-4-semidiff-pipe, OutBufferAccept (%p):\n", outBufferAccept);
