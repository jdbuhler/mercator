#ifndef __SELFLOOP_DRIVER_CU
#define __SELFLOOP_DRIVER_CU
...
//#include "../codegenInput/SelfLoop.cuh"

#if MAPPING_1TO1
  #include "../codegenInput/SelfLoop_1to1map.cuh"
  #define APP_TYPE SelfLoop_1to1map
#elif MAPPING_1TO2
  #include "../codegenInput/SelfLoop_1to2map.cuh"
  #define APP_TYPE SelfLoop_1to2map
#elif MAPPING_1TO4
  #include "../codegenInput/SelfLoop_1to4map.cuh"
  #define APP_TYPE SelfLoop_1to4map
#elif MAPPING_2TO1
  #include "../codegenInput/SelfLoop_2to1map.cuh"
  #define APP_TYPE SelfLoop_2to1map
#elif MAPPING_4TO1
  #include "../codegenInput/SelfLoop_4to1map.cuh"
  #define APP_TYPE SelfLoop_4to1map
#else
  #error "INVALID MAPPING SELECTION"  
#endif

#define PRINT_INPUT_BUFFER_SELFLOOPPIPE 0
#define PRINT_OUTPUT_BUFFERS_SELFLOOPPIPE 0
...
void run_selfLoop()
{
  ...
  // fill input buffer
  // NB: select correct numIters based on desired mapping strategy
  srand(inputSeed);
...
#if PRINT_INPUT_BUFFER_SELFLOOPPIPE
  ...
  // create app object
  APP_TYPE* selfLoop = new APP_TYPE();
...
//    printf("Setting upper bound of node %d to %d\n", i, upperBd);
  ...
  selfLoop->set_nodeUserData(APP_TYPE::Node::Anode, myNodeData);

  // associate buffers with nodes
  selfLoop->sourceNode->set_inBuffer(inBuffer);
  selfLoop->sinkNodeAccept->set_outBuffer(outBufferAccept);

  // run main function
  selfLoop->run();

  std::cout << "SELF-LOOP APP FINISHED.\n" ;
...
#if PRINT_OUTPUT_BUFFERS_SELFLOOPPIPE
  std::cout << " Output buffer: \n" ;

  printf("Self-loop, OutBufferAccept (%p):\n", outBufferAccept);
