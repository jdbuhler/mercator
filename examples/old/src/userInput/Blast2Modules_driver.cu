#ifndef __BLAST2MODULES_DRIVER_CU
#define __BLAST2MODULES_DRIVER_CU
...
#if MAPPING_1TO1
  #include "../codegenInput/Blast2ModulesApp_1to1map.cuh"
  #define APP_TYPE Blast2ModulesApp_1to1map
#else
...
//	  sinkNodeNameVec.push_back(sinkNodeName);
...
#if 0
	  // fill input buffer: sequential db indices
	...
	//for(int i=0; i < inBuffer->get_numItems(); ++i)
	 // std::cout << "Input[" << i << "]: " << inBuffer->peek(i) << "\n";
	  ...
	  // create app object
          APP_TYPE* blast2ModulesApp = new APP_TYPE();
		...
		*/
		...
		BlastData* blast2ModulesAppData = new BlastData(qHits, qHash, pos, HASH_SIZE, q, d, qSize + STRING_BUFF * 8, dSize + STRING_BUFF * 2);
		blast2ModulesApp->set_userData(blast2ModulesAppData);


  // associate buffers with nodes
  blast2ModulesApp->sourceNode->set_inBuffer(inBuffer);
  blast2ModulesApp->sinkNodeAccept->set_outBuffer(outBuff);

// print contents of output buffer
std::cout << "Calling run fcn for blast app... \n" ;

  // run main function
  blast2ModulesApp->run();
