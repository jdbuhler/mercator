#ifndef __REPDIFFPIPE_DRIVER_CU
#define __REPDIFFPIPE_DRIVER_CU
...
#if MAPPING_1TO1
  #include "../codegenInput/RepDiffPipe_1to1map.cuh"
  #define APP_TYPE RepDiffPipe_1to1map
//#elif MAPPING_1TO2
...
#define PRINT_INPUT_BUFFER_REPDIFFPIPE 0
#define PRINT_OUTPUT_BUFFERS_REPDIFFPIPE 0

void run_repDiffPipe()
{
...
//  constexpr int GAMMA = 2;
...
#if PRINT_INPUT_BUFFER_REPDIFFPIPE 
  // print input buffer contents
  printf("RepDiffPipe, InputBuffer (%p):\n", inBuffer);
  for(int i=0; i < IN_BUFFER_CAPACITY; ++i)
  {
  ...
  // create app object
  APP_TYPE* repDiffPipe = new APP_TYPE();
...
//    printf("Setting upper bound of node %d to %d\n", i, upperBd);
  ...
  repDiffPipe->A->set_nodeUserData(APP_TYPE::A::Node::A1node, myNodeData[0]);
  repDiffPipe->B->set_nodeUserData(APP_TYPE::B::Node::B1node, myNodeData[1]);
  repDiffPipe->C->set_nodeUserData(APP_TYPE::C::Node::C1node, myNodeData[2]);
  repDiffPipe->D->set_nodeUserData(APP_TYPE::D::Node::D1node, myNodeData[3]);
  repDiffPipe->E->set_nodeUserData(APP_TYPE::E::Node::E1node, myNodeData[4]);
  repDiffPipe->A->set_nodeUserData(APP_TYPE::A::Node::A2node, myNodeData[0]);
  repDiffPipe->B->set_nodeUserData(APP_TYPE::B::Node::B2node, myNodeData[1]);
  repDiffPipe->C->set_nodeUserData(APP_TYPE::C::Node::C2node, myNodeData[2]);
  repDiffPipe->D->set_nodeUserData(APP_TYPE::D::Node::D2node, myNodeData[3]);
  repDiffPipe->E->set_nodeUserData(APP_TYPE::E::Node::E2node, myNodeData[4]);

  // associate buffers with nodes
  repDiffPipe->sourceNode->set_inBuffer(inBuffer);
  repDiffPipe->sinkNodeAccept1->set_outBuffer(outBufferAccept);
  repDiffPipe->sinkNodeAccept2->set_outBuffer(outBufferAccept);

  // run main function
  repDiffPipe->run();

  std::cout << "REP-DIFF-PIPE APP FINISHED.\n" ;
  ...
  printf("Rep-diff-pipe, OutBufferAccept (%p):\n", outBufferAccept);
