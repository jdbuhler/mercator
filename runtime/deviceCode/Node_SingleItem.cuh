...
namespace Mercator  {
  ...
  protected:
    ...
    void fire()
    {
      ...
      // of ensemble width.  Set further downstream depending on if we
      // have singals
      ...
      while (nTotalConsumed < nTotalToConsume && mynDSActive == 0 && 
	     !dsSignalFull)
	{
	  // # of items already consumed from queue
	  unsigned int nConsumed = 0;
	  unsigned int nToConsume = queue.getOccupancy()- nTotalConsumed;
	  ...
	  if (this->numSignalsPending() > 0) 
		...
		this->currentCreditCounter -= nConsumed;
	      ...
	      //Syncthreads before entering the signal handeler, need
	      //to make sure that every thread knows the current
	      //consumed totals.
	      __syncthreads();
